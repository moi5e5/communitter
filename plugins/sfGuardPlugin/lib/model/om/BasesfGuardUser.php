...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array CommunityItems[] Collection to store aggregation of CommunityItems objects.
	 */
	protected $collCommunityItemss;

	/**
	 * @var        Criteria The criteria used to select the current contents of collCommunityItemss.
	 */
	private $lastCommunityItemsCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collCommunityItemss = null;
			$this->lastCommunityItemsCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collCommunityItemss !== null) {
				foreach ($this->collCommunityItemss as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getCommunityItemss() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addCommunityItems($relObj->copy($deepCopy));
				}
			}
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getsfGuardUserProfilesJoinCommunityTypes($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collsfGuardUserProfiles === null) {
			if ($this->isNew()) {
				...
				$this->collsfGuardUserProfiles = sfGuardUserProfilePeer::doSelectJoinCommunityTypes($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collCommunityItemss collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addCommunityItemss()
	 */
	public function clearCommunityItemss()
	{
		$this->collCommunityItemss = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collCommunityItemss collection (array).
	 *
	 * By default this just sets the collCommunityItemss collection to an empty array (like clearcollCommunityItemss());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initCommunityItemss()
	{
		$this->collCommunityItemss = array();
	}

	/**
	 * Gets an array of CommunityItems objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related CommunityItemss from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array CommunityItems[]
	 * @throws     PropelException
	 */
	public function getCommunityItemss($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collCommunityItemss === null) {
			if ($this->isNew()) {
			   $this->collCommunityItemss = array();
				...
				$criteria->add(CommunityItemsPeer::USER_ID, $this->id);

				CommunityItemsPeer::addSelectColumns($criteria);
				$this->collCommunityItemss = CommunityItemsPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				CommunityItemsPeer::addSelectColumns($criteria);
				if (!isset($this->lastCommunityItemsCriteria) || !$this->lastCommunityItemsCriteria->equals($criteria)) {
					$this->collCommunityItemss = CommunityItemsPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastCommunityItemsCriteria = $criteria;
		return $this->collCommunityItemss;
	}

	/**
	 * Returns the number of related CommunityItems objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related CommunityItems objects.
	 * @throws     PropelException
	 */
	public function countCommunityItemss(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collCommunityItemss === null) {
			if ($this->isNew()) {
				...
				$count = CommunityItemsPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastCommunityItemsCriteria) || !$this->lastCommunityItemsCriteria->equals($criteria)) {
					...
					$count = count($this->collCommunityItemss);
				}
			} else {
				$count = count($this->collCommunityItemss);
			}
	...
	/**
	 * Method called to associate a CommunityItems object to this object
	 * through the CommunityItems foreign key attribute.
	 *
	 * @param      CommunityItems $l CommunityItems
	 * @return     void
	 * @throws     PropelException
	 */
	public function addCommunityItems(CommunityItems $l)
	{
		if ($this->collCommunityItemss === null) {
			$this->initCommunityItemss();
		}
		if (!in_array($l, $this->collCommunityItemss, true)) { // only add it if the **same** object is not already associated
			array_push($this->collCommunityItemss, $l);
			$l->setsfGuardUser($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collCommunityItemss) {
				foreach ((array) $this->collCommunityItemss as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
